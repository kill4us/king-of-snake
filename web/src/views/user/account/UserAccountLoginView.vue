<template>
    <ContentField v-if="$store.state.user.pulling_info == false">
...
export default {
    ...
    setup() {
        ...
        const jwt_token = localStorage.getItem("jwt_token");  //  从本地localStorage取出token
        if (jwt_token) {
        ...
        const acwing_login = () => {
            $.ajax({
                url: "https://app6690.acapp.acwing.com.cn/api/user/account/acwing/web/apply_code/",
                type: "get",
                success: resp => {
